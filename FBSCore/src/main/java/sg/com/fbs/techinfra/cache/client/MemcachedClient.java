...
public class MemcachedClient {
		...
		public void doMulti(boolean asString,
			...
			try {
				...
				try {
					...
					log.debug("ignore : " +ignoreMe.getCause());
				}
	 ...
	 *            ClassLoader object.
	 */
	public MemcachedClient(ClassLoader classLoader, SockIOPool poolUsed) {
		this.classLoader = classLoader;
		init(poolUsed);
	}
	 ...
	 */
	public MemcachedClient(SockIOPool poolUsed) {
		init(poolUsed);
	}
	...
	/**
	 * This method will return the max size of the cache in bytes.
	 * 
	 * @return
	 */
	public Long getCacheMaxSize() {

		HashMap<String, String> serverStats = getCacheServerStats();
		if(serverStats.size() > 0 && serverStats.containsKey("limit_maxbytes")){
			return new Long(serverStats.get("limit_maxbytes"));
		}
		return 0L;
	...
	/**
	 * This method will return the size of cache in bytes.
	 * 
	 * @return cache size
	 */
	public Long getCacheSize() {
		HashMap<String, String> serverStats = getCacheServerStats();
		if(serverStats.size() > 0 && serverStats.containsKey("bytes")){
			return new Long(serverStats.get("bytes"));
		}
		return 0L;
	}
	...
	/**
	 * This method will cache is full or not.
	 */
	public boolean isCacheFull() {

		Long currSize = getCacheSize();
		Long maxSize = getCacheMaxSize();
	...
	/**
	 * This method will return the 1 server in the server list, so if there are
	 * multiple cache servers, please do not use this method
	 * 
	 * @return first cache server stats
	 */
	private HashMap<String, String> getCacheServerStats() {

		HashMap<String, Map> mccStats = (HashMap<String, Map>) stats();
		Iterator<String> itr = mccStats.keySet().iterator();
		HashMap<String, String> serverStats = new HashMap<String, String>();
		if(mccStats.size() > 0){
			serverStats = (HashMap<String, String>) mccStats.get(itr.next());
		}
		return serverStats;
	}
	 ...
	 */
	private void init(SockIOPool poolUsed) {
	...
	public synchronized int getReadCnt() {
		return readCnt;
	}

	private synchronized void incReadCnt() {
		this.readCnt++;
	}
	...
	private synchronized void incHitCnt() {
		this.hitCnt++;
	}
}
